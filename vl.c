...
// ========================================================================
    ...
    #include <windows.h>
    #include <io.h>
    bool os_is_interactive() { return _isatty(_fileno(stdin)); }
    ...
    bool os_is_interactive() { return isatty(STDIN_FILENO); }
...
void tokenize(const char* source) {
    ...
    while (i < len) {
        ...
        if (c == '#') { while (i < len && source[i] != '\n') i++; continue; }
        if (c == '(') { add_token(TOKEN_LPAREN, "(", line); i++; }
        else if (c == ')') { add_token(TOKEN_RPAREN, ")", line); i++; }
        else if (c == '{') { 
            add_token(TOKEN_LBRACE, "{", line); 
            if (template_level > 0) template_brace_depth[template_level]++;
            i++; 
        }
        else if (c == '}') { 
            if (template_level > 0) {
                template_brace_depth[template_level]--;
                if (template_brace_depth[template_level] == 0) {
                    // Akhir dari blok ${...}, tutup dengan ) dan sambung pakai +
                    add_token(TOKEN_RPAREN, ")", line);
                    add_token(TOKEN_PLUS, "+", line);
                    template_level--;
                    i++; 
                    
                    // Langsung paksa baca sisa template literal selanjutnya!
                    int str_len = 0;
                    char* buffer = malloc(len + 1);
                    while (i < len && source[i] != '`' && !(source[i] == '$' && i + 1 < len && source[i+1] == '{')) {
                        if (source[i] == '\n') line++;
                        if (source[i] == '\\' && i + 1 < len) {
                            i++;
                            if (source[i] == '`') buffer[str_len++] = '`';
                            else if (source[i] == '$') buffer[str_len++] = '$';
                            else if (source[i] == 'n') buffer[str_len++] = '\n';
                            else { buffer[str_len++] = '\\'; buffer[str_len++] = source[i]; }
                        } else {
                    ...
                    if (i < len && source[i] == '$' && source[i+1] == '{') {
                        i += 2;
                        add_token(TOKEN_PLUS, "+", line);
                        add_token(TOKEN_LPAREN, "(", line);
                        template_level++;
                        template_brace_depth[template_level] = 1;
                    } else if (i < len && source[i] == '`') {
                        i++;
                    }
                    continue; // Bypass sisa loop, balik ke awal pembacaan token baru
                } else {
                    add_token(TOKEN_RBRACE, "}", line); i++;
                }
            } else {
                add_token(TOKEN_RBRACE, "}", line); i++; 
            }
        }
        else if (c == '[') { add_token(TOKEN_LBRACKET, "[", line); i++; }
        else if (c == ']') { add_token(TOKEN_RBRACKET, "]", line); i++; }
            ...
            int str_len = 0;
            char* buffer = malloc(len + 1);
            while (i < len && source[i] != quote_type) {
                if (source[i] == '\n') line++;
                if (source[i] == '\\' && i + 1 < len) {
                    i++;
                    ...
                    else if (source[i] == 'e') buffer[str_len++] = '\033'; 
                    else if (source[i] == '\\') buffer[str_len++] = '\\';
                    else if (source[i] == quote_type) buffer[str_len++] = quote_type; // Support kutip dinamis
            ...
            // Baca teks sampai ketemu backtick penutup ATAU simbol ${
            while (i < len && source[i] != '`' && !(source[i] == '$' && i + 1 < len && source[i+1] == '{')) {
                if (source[i] == '\n') line++;
                if (source[i] == '\\' && i + 1 < len) {
                    i++;
            ...
            // Kalau berhentinya karena ketemu ${, kita push state dan biarin lexer jalan normal
            if (i < len && source[i] == '$' && source[i+1] == '{') {
                i += 2;
                add_token(TOKEN_PLUS, "+", line);
                add_token(TOKEN_LPAREN, "(", line);
...
struct Expr {
    ...
    union {
        ...
        struct { Expr* object; Expr* index; } index_get;
...
// ASI (Automatic Semicolon Insertion) Logic
bool is_eos() {
    if (check(TOKEN_SEMI)) { 
        advance(); 
        return true; 
    }
    // Langsung tembak is_at_end() di sini biar REPL lu gak error dungu
    if (is_at_end() || check(TOKEN_RBRACE)) { 
        return true; 
    }
    if (current_tok > 0 && previous().line < peek().line) { 
        return true; 
    }
    return false;
}
...
Expr* parse_call() {
    ...
    while (true) {
        if (match(TOKEN_LPAREN)) {
            ...
            if (!match(TOKEN_RBRACKET)) parse_error(peek().line, "Fatal Error: Expected ']' after pointer index.");
            Expr* idx_expr = new_expr(EXPR_INDEX_GET);
            idx_expr->as.index_get.object = expr;
            idx_expr->as.index_get.index = index;
            expr = idx_expr; 
...
Expr* parse_assignment() {
    ...
    if (match(TOKEN_ASSIGN)) {
        ...
        } else if (expr->type == EXPR_INDEX_GET) {
            Expr* set = new_expr(EXPR_INDEX_SET);
            set->as.index_set.object = expr->as.index_get.object;
            set->as.index_set.index = expr->as.index_get.index;
            set->as.index_set.value = value;
            free(expr); return set;
        }
...
Stmt* parse_stmt() {
    ...
    if (match(TOKEN_BREAK)) {
        if (!is_eos()) parse_error(peek().line, "Expected newline or ';' after break.");
    ...
    if (match(TOKEN_VERSE_SWITCH)) {
        ...
        if (check(TOKEN_LBRACE)) {
            ...
            if (!is_eos()) parse_error(peek().line, "Expected newline, ';' or '{' after verse_switch.");
        }
    ...
    if (match(TOKEN_WITH)) {
        ...
        // Kalau user ngetik modul tanpa kutip (terdeteksi sebagai Identifier)
        if (check(TOKEN_IDENT)) {
            path = new_expr(EXPR_LITERAL);
            // Paksa ambil lexeme (teks mentah) dari token dan jadikan string literal
            path->as.literal = make_str(peek().lexeme);
            advance(); // Maju ke token berikutnya biar parser gak nyangkut
        } 
        // Kalau user ngetik pake kutip ("gamengine") atau ekspresi kompleks lain
        else {
            path = parse_expr();
        }
        
        if (!is_eos()) parse_error(peek().line, "Expected newline or ';' after with statement.");
    ...
    if (match(TOKEN_VAR) || match(TOKEN_STVAR)) {
        ...
        if (!is_eos()) parse_error(peek().line, "Expected newline or ';' after var decl.");
    ...
    if (match(TOKEN_RETURN)) {
        Expr* val = NULL;
        if (!check(TOKEN_SEMI) && !check(TOKEN_RBRACE) && !is_at_end() && previous().line == peek().line) {
            val = parse_expr();
        }
        if (!is_eos()) parse_error(peek().line, "Expected newline or ';' after return.");
        Stmt* s = new_stmt(STMT_RETURN); s->as.expr = val; return s;
    }
    
    Expr* expr = parse_expr();
    if (!is_eos()) parse_error(peek().line, "Expected newline or ';' after expression.");
...
Value eval(Expr* expr) {
    ...
    if (expr->type == EXPR_INDEX_GET) {
        Value obj = eval(expr->as.index_get.object);
        Value idx = eval(expr->as.index_get.index);
        
        if (obj.type == VAL_PTR) {
            if (!in_unsafe_block) runtime_error("Fatal Error: Raw pointer memory access is strictly forbidden outside 'unsafe {}' block.");
            if (idx.type != VAL_NUM) runtime_error("Fatal Error: Memory offset index must be a numeric integer.");
            
            uint8_t* raw_mem = (uint8_t*)obj.as.ptr;
            int offset = (int)idx.as.num;
            return make_num((double)raw_mem[offset]);
        }
        runtime_error("Fatal Error: Array indexing is currently only supported for raw memory pointers (VAL_PTR).");
    }

    if (expr->type == EXPR_INDEX_SET) {
        Value obj = eval(expr->as.index_set.object);
        Value idx = eval(expr->as.index_set.index);
        Value val = eval(expr->as.index_set.value);
        
        if (obj.type == VAL_PTR) {
            if (!in_unsafe_block) runtime_error("Fatal Error: Raw pointer mutation is strictly forbidden outside 'unsafe {}' block.");
            if (idx.type != VAL_NUM || val.type != VAL_NUM) runtime_error("Fatal Error: Memory offset and value must be numeric.");
            
            uint8_t* raw_mem = (uint8_t*)obj.as.ptr;
            int offset = (int)idx.as.num;
            raw_mem[offset] = (uint8_t)val.as.num;
            return val;
        }
        runtime_error("Fatal Error: Index assignment is only supported for raw pointers.");
    }
    ...
    if (expr->type == EXPR_BINARY) {
        ...
        if (op == TOKEN_PLUS) {
            ...
            if (left.type == VAL_STR || right.type == VAL_STR) {
                char* s1 = left.type == VAL_STR ? left.as.str : NULL;
                char* s2 = right.type == VAL_STR ? right.as.str : NULL;
                
                char buf1[256], buf2[256];
                if (!s1) { snprintf(buf1, sizeof(buf1), "%g", left.as.num); s1 = buf1; }
                if (!s2) { snprintf(buf2, sizeof(buf2), "%g", right.as.num); s2 = buf2; }
                
                // Alokasi memori secara dinamis berdasarkan panjang string aktual!
                char* comb = malloc(strlen(s1) + strlen(s2) + 1);
                sprintf(comb, "%s%s", s1, s2); 
                Value res = make_gc_str(comb); 
                free(comb);
                return res;
            }
...
Value native_array(int argCount, Value* args) {
    ...
    return track_obj(make_array(arr)); 
}
...
Value native_int(int argCount, Value* args) {
    ...
    if (args[0].type == VAL_NUM) {
        return make_num((int)args[0].as.num); 
    } else if (args[0].type == VAL_STR) {
...
Value native_str(int argCount, Value* args) {
    if (argCount != 1) runtime_error("str() requires exactly 1 argument.");
    ...
    if (v.type == VAL_NUM) {
        snprintf(buffer, sizeof(buffer), "%g", v.as.num);
    } else if (v.type == VAL_BOOL) {
        snprintf(buffer, sizeof(buffer), "%s", v.as.boolean ? "yes" : "no");
        ...
        snprintf(buffer, sizeof(buffer), "<fc>");
    } else if (v.type == VAL_MODULE) {
        snprintf(buffer, sizeof(buffer), "<module>");
        ...
        snprintf(buffer, sizeof(buffer), "<raw_ptr %p>", v.as.ptr);
    } else {
...
Value decode_to_ascii(Value* args, int base, const char* func_name) {
    ...
    char* input = strdup(args[0].as.str); 
...
Value native_require_cpu_instruction(int argCount, Value* args) {
    if (!in_unsafe_block) runtime_error("Fatal Error: Hardware architecture verification must be declared inside 'unsafe {}'.");
    if (argCount != 1 || args[0].type != VAL_STR) runtime_error("Fatal Error: require_cpu_instruction() needs 1 string argument (e.g., 'avx2').");
    
    const char* instruction = args[0].as.str;
    bool supported = false;
        ...
        if (strcmp(instruction, "avx2") == 0) {
            if (sysctlbyname("hw.optional.avx2", &val, &len, NULL, 0) == 0 && val == 1) {
                supported = true;
            }
        } else if (strcmp(instruction, "sse4.2") == 0) {
            if (sysctlbyname("hw.optional.sse4_2", &val, &len, NULL, 0) == 0 && val == 1) {
                supported = true;
            }
        } else {
            char err[256];
            snprintf(err, sizeof(err), "Hardware Error: Unrecognized macOS CPU flag: '%s'.", instruction);
            runtime_error(err);
        }

    #elif defined(__GNUC__) && !defined(__clang__)
        
        // PAGAR ARSITEKTUR 1: Hanya jalankan ini kalau CPU-nya Intel/AMD x86_64 atau 32-bit
            ...
            if (strcmp(instruction, "avx2") == 0) {
                ...
                char err[256];
                snprintf(err, sizeof(err), "Hardware Error: Unrecognized x86 CPU flag: '%s'.", instruction);
                runtime_error(err);
            }
            
        // PAGAR ARSITEKTUR 2: Kalau CPU-nya Keluarga ARM (Misal Linux ARM64 lu!)
        #elif defined(__aarch64__) || defined(__arm__)
            if (strcmp(instruction, "neon") == 0) {
                supported = true; // ARM64 modern biasanya selalu support instruksi NEON
            } else if (strcmp(instruction, "avx2") == 0 || strcmp(instruction, "sse4.2") == 0) {
                supported = false; // ARM jelas gak punya instruksi x86!
            } else {
                char err[256];
                snprintf(err, sizeof(err), "Hardware Error: Unrecognized ARM CPU flag: '%s'.", instruction);
                runtime_error(err);
            }
            
        // PAGAR ARSITEKTUR 3: Arsitektur lain (RISC-V, MIPS, dll) otomatis *fallback* aman
        #else
            if (strcmp(instruction, "avx2") == 0 || strcmp(instruction, "sse4.2") == 0) {
                supported = false;
            } else {
                supported = true; // Bypass untuk arsitektur gaib
            }
        #endif
    ...
    if (!supported) {
        char err[256];
        snprintf(err, sizeof(err), "Hardware Error: Target CPU architecture does not support %s instruction set.", instruction);
        runtime_error(err);
    }
...
double simulated_registers[16] = {0}; 

int get_register_index(const char* reg_name) {
    // Dukungan Register x86_64 (Intel/AMD)
    if (strcmp(reg_name, "RAX") == 0) return 0;
    if (strcmp(reg_name, "RBX") == 0) return 1;
    if (strcmp(reg_name, "RCX") == 0) return 2;
    if (strcmp(reg_name, "RDX") == 0) return 3;
    
    // Dukungan Register ARM64 (Tambahan anarki buat ekosistem baru lu!)
    if (strcmp(reg_name, "X0") == 0) return 0;
    if (strcmp(reg_name, "X1") == 0) return 1;
    if (strcmp(reg_name, "X2") == 0) return 2;
    if (strcmp(reg_name, "X3") == 0) return 3;

    runtime_error("Hardware Error: Unknown or unauthorized CPU register mapped.");
    return -1;
}

Value native_set_cpu_register(int argCount, Value* args) {
    if (!in_unsafe_block) runtime_error("Fatal Error: CPU register mutation requires 'unsafe {}' block.");
    if (argCount != 2 || args[0].type != VAL_STR || args[1].type != VAL_NUM) runtime_error("Fatal Error: set_cpu_register() requires (String, Number).");
    
    int idx = get_register_index(args[0].as.str);
    simulated_registers[idx] = args[1].as.num;
    return make_nil();
}

Value native_get_cpu_register(int argCount, Value* args) {
    if (!in_unsafe_block) runtime_error("Fatal Error: CPU register reading requires 'unsafe {}' block.");
    if (argCount != 1 || args[0].type != VAL_STR) runtime_error("Fatal Error: get_cpu_register() requires 1 String argument.");
    
    int idx = get_register_index(args[0].as.str);
    return make_num(simulated_registers[idx]);
}
...
void inject_standard_library(Environment* env) {
    ...
    define_var(env, "require_cpu_instruction", make_native(native_require_cpu_instruction), false);
    define_var(env, "set_cpu_register", make_native(native_set_cpu_register), false);
    define_var(env, "get_cpu_register", make_native(native_get_cpu_register), false);
    ...
    define_var(env, "str", make_native(native_str), false);
...
int main(int argc, char** argv) {
    ...
    if (argc == 1) {
        if (os_is_interactive()) {
            // MODE REPL INTERAKTIF ASLI LU
            ...
            // MODE PIPE: SIKAT SEMUA DATA DARI STDIN TANPA PROMPT
            int cap = 1024;
            int len = 0;
            char* buffer = malloc(cap);
            int ch;
            
            // Sedot biner sampai ketemu End Of File (EOF)
            ...
            // Langsung eksekusi payload biner-nya
            run_source(buffer);
            free(buffer);
        }
    } else if (argc >= 2) {
        if (strcmp(argv[1], "compile") == 0) {
            ...
            char* source = read_file_system(input_file);
            compile_source_to_file(source, output_file);
            free(source);
        } 
        else if (strcmp(argv[1], "-byte") == 0) {
            if (argc < 3) { printf("Usage: vl -byte <file.vlb>\n"); return 64; }
            // Langsung eksekusi dari input user secara mentah
            run_bytecode_file(argv[2]);
        }
        else {
            // Langsung baca file secara eksplisit
            char* source = read_file_system(argv[1]); 
            run_source(source); 
            free(source);
        }
    }
