...
typedef enum { VAL_NIL, VAL_NUM, VAL_BOOL, VAL_STR, VAL_NATIVE, VAL_FUNC, VAL_MODULE, VAL_ARRAY, VAL_CLASS, VAL_INSTANCE } ValType;
...
typedef enum { EXPR_LITERAL, EXPR_VAR, EXPR_ASSIGN, EXPR_BINARY, EXPR_UNARY, EXPR_CALL, EXPR_GET, EXPR_SET, EXPR_INCREMENT } ExprType;
struct Expr {
    ExprType type;
    union {
        ...
        struct { Expr* klass_expr; Expr** args; int arg_count; } new_expr; // Untuk 'new'
        struct { Expr* object; char* property; Expr* value; } set;         // Untuk 'this.nama = x'
        struct { Expr* var_expr; } increment;
    } as;
};

typedef enum { STMT_VAR, STMT_BLOCK, STMT_IF, STMT_LOOP_NUMBER, STMT_LOOP_INF, STMT_FUNC, STMT_RETURN, STMT_WITH, STMT_EXPR, STMT_CLASS, STMT_SWITCH, STMT_BREAK, STMT_VERSE_SWITCH } StmtType;
...
Expr* parse_call() {
    ...
    while (true) {
        if (match(TOKEN_LPAREN)) {
            // ... (biarkan blok arguments LPAREN ini utuh tidak diubah) ...
            int cap = 4, count = 0; Expr** args = malloc(sizeof(Expr*) * cap);
            if (!check(TOKEN_RPAREN)) {
                do {
                    if (count >= cap) { cap *= 2; args = realloc(args, sizeof(Expr*) * cap); }
            ...
            // ... (biarkan blok DOT property ini utuh) ...
        ...
        } else if (match(TOKEN_PLUS_PLUS)) { // <-- TAMBAHKAN LOGIKA TANGKAP ++ DI SINI!
            Expr* inc = new_expr(EXPR_INCREMENT);
            inc->as.increment.var_expr = expr;
            expr = inc; // Timpa ekspresi dengan operasi increment
        } else { break; }
    }
    return expr;
}
...
Value eval(Expr* expr) {
    ...
    // ==========================================
    // EKSEKUSI OPERATOR ++ (POST-INCREMENT)
    // ==========================================
    if (expr->type == EXPR_INCREMENT) {
        Expr* target = expr->as.increment.var_expr;
        
        // Kasus 1: Increment Variabel Biasa (contoh: x++)
        if (target->type == EXPR_VAR) {
            Value val = get_var(current_env, target->as.name);
            if (val.type != VAL_NUM) runtime_error("Operator ++ hanya bisa untuk angka!");
            
            Value new_val = make_num(val.as.num + 1);
            assign_var(current_env, target->as.name, new_val);
            return val; // Kembalikan nilai LAMA (khas bahasa C++)
        } 
        // Kasus 2: Increment Properti Kelas (contoh: this.x++)
        else if (target->type == EXPR_GET) {
            Value obj = eval(target->as.get.object);
            if (obj.type != VAL_INSTANCE) runtime_error("Target ++ is invalid!");
            
            EnvNode* node = obj.as.instance->fields->head;
            while(node) {
                if (strcmp(node->name, target->as.get.property) == 0) {
                    if (node->value.type != VAL_NUM) runtime_error("The ++ operator only works with numbers!");
                    Value old_val = node->value;
                    node->value = make_num(old_val.as.num + 1); // Timpa memori di RAM!
                    return old_val; 
                }
                node = node->next;
            }
            runtime_error("No property found to increment.");
        }
        runtime_error("Invalid target for operator ++.");
    }
    // ==========================================
...
void inject_standard_library(Environment* env) {
    ...
    // 2. Decoder Number (Hex/Oct/Bin -> Angka Desimal)
    ...
    // 3. Decoder ASCII (Hex/Oct/Bin -> Teks ASCII Mentah)
    ...
    // --- INJEKSI KEKUATAN MULTIVERSE KE SEMUA DIMENSI ---
...
int main(int argc, char** argv) {
    ...
    int arg_start = 2;
    if (argc >= 2) {
        if (strcmp(argv[1], "compile") == 0) arg_start = 3;
        else if (strcmp(argv[1], "-byte") == 0) arg_start = 3;
    }
    for (int i = arg_start; i < argc; i++) {
        if (i < argc) sys_args_arr->elements[sys_args_arr->count++] = track_obj(make_str(argv[i]));
    }
