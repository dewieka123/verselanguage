...
int main(int argc, char** argv) {
    ...
    if (argc == 1) {
        ...
        // Cek apakah file dengan nama asli ada di sistem
        FILE* test_file = fopen(filepath, "rb");
        if (!test_file) {
            // Jika file tidak ada, dan tidak berakhiran .vl, tempelkan .vl
            int len = strlen(filepath);
            if (len < 3 || strcmp(filepath + len - 3, ".vl") != 0) {
                snprintf(filepath, sizeof(filepath), "%s.vl", argv[1]);
            }
        } else {
            // Jika ada, tutup filenya agar tidak memakan memori, kita hanya menguji keberadaannya
            fclose(test_file);
        }

        // Sekarang jalankan file berdasarkan path yang sudah dipastikan valid
