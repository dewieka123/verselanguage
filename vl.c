...
typedef struct { TokenType type; char* lexeme; int line; } Token;

Token* tokens = NULL;
int token_count = 0;
int token_capacity = 0;

void add_token(TokenType type, const char* lexeme, int line) {
...
TokenType check_keyword(const char* ident) {
...
Expr* parse_binary(Expr* (*next)(), TokenType* ops, int op_count) {
    Expr* expr = next();
    while (true) {
        bool matched = false;
