...
typedef enum {
    ...
    TOKEN_YES, TOKEN_NO, TOKEN_CLASS, TOKEN_NEW, TOKEN_THIS, TOKEN_EOF
} VL_TokenType;
...
typedef enum { EXPR_LITERAL, EXPR_VAR, EXPR_ASSIGN, EXPR_BINARY, EXPR_UNARY, EXPR_CALL, EXPR_GET, EXPR_SET } ExprType;
struct Expr {
    ExprType type;
    union {
...
typedef enum { STMT_VAR, STMT_BLOCK, STMT_IF, STMT_LOOP_NUMBER, STMT_LOOP_INF, STMT_FUNC, STMT_RETURN, STMT_WITH, STMT_EXPR, STMT_CLASS } StmtType;
struct Stmt {
    StmtType type;
    union {
        Expr* expr;
...
int main(int argc, char** argv) {
    ...
    define_var(global_env, "color", make_native(native_color), false);
    ...
    // Deteksi dari mana argumen pengguna dimulai (Bypass flag vl, compile, atau -byte)
    int arg_start = 2; // Default untuk: vl skrip.vl arg1
    if (argc >= 2) {
        if (strcmp(argv[1], "compile") == 0) arg_start = 3;       // vl compile skrip.vl arg1
        else if (strcmp(argv[1], "-byte") == 0) arg_start = 3;   // vl -byte skrip.vlb arg1
    }
    
    // Sedot semua argumen terminal jadi string dan masukkan ke Array GC kita!
    for (int i = arg_start; i < argc; i++) {
        if (i < argc) { // Keamanan memori brutal
            sys_args_arr->elements[sys_args_arr->count++] = track_obj(make_str(argv[i]));
        }
    }
    
    // Daftarkan 'sys_args' sebagai variabel Array global!
    define_var(global_env, "sys_args", track_obj(make_array(sys_args_arr)), false);
    // ====================================================================
