...
void execute(Stmt* stmt) {
    ...
    if (stmt->type == STMT_VAR) {
        ...
        for (int i = 0; i < count.as.num; i++) {
            ...
            gc_collect(); // SAPU BERSIH SETELAH 1 SIKLUS ARRAY EKOR!
        }
...
void compile_source_to_file(const char* source, const char* out_filepath) {
    ...
    // Tulis Header Identitas Bytecode VerseLanguage
    fwrite("VLB\0", sizeof(char), 4, file); 
    
    // Tulis Panjang Kode Sumber
    int len = strlen(source);
    fwrite(&len, sizeof(int), 1, file); 
    
    // Enkripsi Teks Menggunakan Manipulasi Biner XOR (Kunci: 0x4B)
    for (int i = 0; i < len; i++) {
        uint8_t encrypted = (uint8_t)source[i] ^ 0x4B; 
...
void run_bytecode_file(const char* filepath) {
    ...
    if (memcmp(header, "VLB\0", 4) != 0) {
        printf("Error: File '%s' is not a valid Verse Bytecode format.\n", filepath);
        fclose(file); exit(74);
    }
    
    // Baca Ukuran Teks Asli
    int len; fread(&len, sizeof(int), 1, file);
    char* source = malloc(len + 1);
    
    // Dekripsi Biner ke Teks Semula
    for (int i = 0; i < len; i++) {
        uint8_t decrypted; fread(&decrypted, sizeof(uint8_t), 1, file);
        source[i] = (char)(decrypted ^ 0x4B); 
    }
    source[len] = '\0';
    fclose(file);
    
    // Lempar langsung teks yang sudah pulih ke AST Evaluator asli!
    run_source(source); 
    free(source);
}
...
int main(int argc, char** argv) {
    ...
    // Registrasi Sistem Array C Murni
