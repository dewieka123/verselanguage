...
Value eval(Expr* expr) {
    ...
    if (expr->type == EXPR_INDEX_SET) {
        ...
        if (obj.type == VAL_PTR) {
            ...
            uint8_t* raw_mem = (uint8_t*)obj.as.ptr;
            int offset = (int)idx.as.num;
            raw_mem[offset] = (uint8_t)val.as.num; // Casting brutal ke ukuran 1 byte
            return val;
        }
...
Value native_require_cpu_instruction(int argCount, Value* args) {
    ...
    #ifdef __GNUC__
    // Deteksi brutal menggunakan compiler GCC/Clang built-in
    if (strcmp(instruction, "avx2") == 0) {
        if (!__builtin_cpu_supports("avx2")) runtime_error("Hardware Error: Target CPU architecture does not support AVX2 instruction set.");
    } else if (strcmp(instruction, "sse4.2") == 0) {
        if (!__builtin_cpu_supports("sse4.2")) runtime_error("Hardware Error: Target CPU architecture does not support SSE4.2 instruction set.");
    } else {
        char err[256];
        snprintf(err, sizeof(err), "Hardware Error: Unrecognized or unsupported CPU instruction flag: '%s'.", instruction);
        runtime_error(err);
    }
    #else
        runtime_error("Hardware Error: CPU instruction verification requires GCC/Clang compiler target.");
    #endif
    
    return make_nil();
}

// Simulasi Manajemen Register di Heap (Agar variabel tidak bercampur)
double simulated_registers[16] = {0}; // 0=RAX, 1=RBX, 2=RCX, 3=RDX, dll
